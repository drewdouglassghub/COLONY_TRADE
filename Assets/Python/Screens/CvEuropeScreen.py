...
import CustomTradeMarkets as TradeMarkets

# globals
EuropeScreenInputMap = {}
gc = CyGlobalContext()
...
class CvEuropeScreen:
	...
	def __init__(self):
		...
		self.TREASURY_ID = 11
		# self.TRADE_WITH_EUROPE_BUTTON_ID = 9988  # Unique ID
	...
	def interfaceScreen(self):
		...
		# Trade Dropdown	
		EuropeScreenInputMap["TradeMarketDropdown"] = self.handleTradeMarketDropdown
		screen.addDropDownBoxGFC("TradeMarketDropdown", 2000, 100, 200, 30, WidgetTypes.WIDGET_GENERAL, -1, FontTypes.SMALL_FONT)
		screen.appendListBoxString("TradeMarketList", "London", WidgetTypes.WIDGET_GENERAL, 0, -1, CvUtil.FONT_LEFT_JUSTIFY)
		screen.show("TradeMarketDropdown")
	...
	def drawContents(self):
		...
		# Button
		# screen.setLabel("TestLabel", "Background", "Button should be here", CvUtil.FONT_CENTER_JUSTIFY, 200, 100, 0, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
		# screen.setButtonGFC("TradeWithEuropeButton", "Trade with Europe", self.X_SCREEN + self.W_SCREEN - 220, self.Y_SCREEN + self.H_SCREEN - 50, 200, 30, WidgetTypes.WIDGET_GENERAL, TRADE_WITH_EUROPE_BUTTON_ID, -1, ButtonStyles.BUTTON_STYLE_STANDARD)
		# screen.show("TradeWithEuropeButton")
...
#			screen.setImageButtonAt(self.getNextWidgetName(), "LoadingList", ArtFileMgr.getInterfaceArtInfo("INTERFACE_EUROPE_SAIL").getPath(), - (self.CARGO_ICON_SIZE / 3) + 4, yLocation_InPort + (self.SHIP_ICON_SIZE * 2) - (self.CARGO_ICON_SIZE * 3 / 4), self.CARGO_ICON_SIZE * 3 / 2, self.CARGO_ICON_SIZE * 3 / 2, WidgetTypes.WIDGET_GENERAL, self.SAIL_TO_NEW_WORLD, unit.getID())
	...
	def handleTradeMarketDropdown(self):
		screen = self.getScreen()
		selectedIndex = screen.getSelectedPullDownID("TradeMarketDropdown")
		selectedCiv = list(TradeMarkets.TRADE_MARKETS.keys())[selectedIndex]
		self.currentTradeMarket = selectedCiv
	...
	def handleInput(self, inputClass):
		...
		if (inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED):
			...
			if (inputClass.getButtonType() == WidgetTypes.WIDGET_GENERAL):
				...
				# elif (inputClass.getData1() == self.TRADE_WITH_EUROPE_BUTTON_ID) :
				# 	player = gc.getPlayer(gc.getGame().getActivePlayer())
				# 	transport = player.getUnit(inputClass.getData2())
				# 	(unit, iter) = player.firstUnit()
				# 	while (unit):
				# 		if (unit.getUnitTravelState() == UnitTravelStates.UNIT_TRAVEL_STATE_IN_EUROPE and unit.isCargo() and unit.isGoods()):
				# 			if (unit.getTransportUnit().getID() == transport.getID()):
				# 				CyMessageControl().sendPlayerAction(player.getID(), PlayerActionTypes.PLAYER_ACTION_TRADE_EUROPE_ANY, 0, unit.getYieldStored(), unit.getID())
				# 		(unit, iter) = player.nextUnit(iter)
	...
	# 		"Selected trade partner: %s" % (TradeMarkets.TRADE_MARKETS[selectedCiv]['name']),
	# 		"", 0, "", gc.getInfoTypeForString("COLOR_WHITE"), 0, 0, False, False)
